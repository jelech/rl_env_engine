...
try:
    from rl_env_engine.generated import simulation_pb2, simulation_pb2_grpc
    ...
    try:
        ...
        try:
            ...
            raise ImportError(
                "Cannot import simulation_pb2. Generate it via protoc or ensure package is installed."
            ) from e
...
class GrpcEnv(gym.Env):
    ...
    def _setup_spaces(self):
        ...
        try:
            ...
            self.action_space = self._convert_proto_space_to_gym(
                response.action_space, is_action_space=True
            )
            self.observation_space = self._convert_proto_space_to_gym(
                response.observation_space, is_action_space=False
            )
            ...
            print(
                f"Warning: Could not get spaces from server for scenario '{self.scenario}': {e}"
            )
    ...
    def _convert_proto_space_to_gym_box(
        self, proto_space, is_action_space: bool = False
    ) -> gym.Space:
        ...
        if not proto_space.low or len(proto_space.low) == 0:
            if (
                is_action_space
                and hasattr(proto_space, "discrete_values")
                and proto_space.discrete_values
            ):
            ...
            if len(low_arr) != shape[0] if len(shape) > 0 else 1:
                ...
                return spaces.Box(
                    ...
                    shape=shape,
                    dtype=getattr(np, proto_space.dtype)
                    if proto_space.dtype
                    else np.float32,
                )
    ...
    def _convert_proto_space_to_gym(
        self, proto_space, is_action_space: bool = False
    ) -> gym.Space:
    ...
    def _create_environment(self):
        ...
        request = simulation_pb2.CreateEnvironmentRequest(
            env_id=self.env_id, scenario=self.scenario, config=config_str
        )
        response = self.client.CreateEnvironment(request)
        if not response.success:
            raise RuntimeError(
                f"Failed to create environment '{self.scenario}': {response.message}"
            )

        self._env_created = True
        self.verbose_print(
            f"Environment created: {self.env_id} (scenario: {self.scenario})"
        )
    ...
    def step(
        ...
        request = simulation_pb2.StepEnvironmentRequest(
            env_id=self.env_id, actions=grpc_actions
        )
    ...
    def _convert_single_action_to_proto_cached(self, action):
        """带缓存的动作转换（适用于离散动作）"""
        if (
            isinstance(action, (int, float, bool))
            and len(self._action_cache) < self._max_cache_size
        ):
            cache_key = (type(action), action)
            if cache_key not in self._action_cache:
                self._action_cache[cache_key] = self._convert_single_action_to_proto(
                    action
                )
    ...
    def _convert_actions_to_proto(
        self, actions: Union[int, float, np.ndarray, list]
    ) -> list:
    ...
    def _convert_single_action_to_proto(
        self, action: Union[int, float, np.ndarray]
    ) -> simulation_pb2.Action:
        ...
        if action.size == 1:
            ...
            if action.dtype in [np.float32, np.float64]:
                return simulation_pb2.Action(
                    float_array=simulation_pb2.FloatArray(
                        values=action.astype(np.float64).tolist()
                    )
                )
            elif action.dtype in [np.int32, np.int64]:
                return simulation_pb2.Action(
                    int_array=simulation_pb2.IntArray(
                        values=action.astype(np.int64).tolist()
                    )
                )
            elif action.dtype == np.bool_:
                return simulation_pb2.Action(
                    bool_array=simulation_pb2.BoolArray(values=action.tolist())
                )
    ...
    def _handle_sequence_action(
        self, action: Union[list, tuple]
    ) -> simulation_pb2.Action:
        """处理列表/元组动作"""
        if not action:
        ...
        if all(isinstance(x, (int, np.integer)) for x in action):
            return simulation_pb2.Action(
                int_array=simulation_pb2.IntArray(values=[int(x) for x in action])
            )
        elif all(isinstance(x, (float, np.floating)) for x in action):
            return simulation_pb2.Action(
                float_array=simulation_pb2.FloatArray(values=[float(x) for x in action])
            )
        elif all(isinstance(x, bool) for x in action):
            return simulation_pb2.Action(
                bool_array=simulation_pb2.BoolArray(values=list(action))
            )
        else:
            try:
                float_values = [float(x) for x in action]
                return simulation_pb2.Action(
                    float_array=simulation_pb2.FloatArray(values=float_values)
                )
            except (ValueError, TypeError):
                raise ValueError(
                    f"Cannot convert mixed-type action to uniform array: {action}"
                )
    ...
    def _fallback_action_conversion(self, action) -> simulation_pb2.Action:
        ...
        try:
            ...
            raise ValueError(
                f"Cannot convert action of type {type(action)} to protobuf Action: {action}"
            )
