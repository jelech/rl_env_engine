...
try:
    from rl_env_engine.generated import simulation_pb2, simulation_pb2_grpc  # type: ignore
except Exception:  # pragma: no cover
    try:
        from ..generated import simulation_pb2, simulation_pb2_grpc  # type: ignore
    except ImportError:
        try:
    ...
    def create_environment(self, env_id, scenario, config=None):
        ...
        try:
            ...
            request = simulation_pb2.CreateEnvironmentRequest(
                env_id=env_id, scenario=scenario, config=config
            )
    ...
    try:
        ...
        if reset_result:
            ...
            if reset_result["observations"]:
                print(
                    f"第一个观察的数据长度: {len(reset_result['observations'][0]['data'])}"
                )

        # 4. 执行几步仿真
        print("\n=== 执行仿真步骤 ===")
        for step in range(3):
            action_value = 1.0 + step * 0.5
