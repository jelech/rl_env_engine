...
class SimulationSDK:
    ...
    def create_sessions(
        ...
        session_infos = {
            uuid.uuid4().hex: self.workers[i % len(self.workers)]
            for i in range(actual_batch)
        }
        
        # 并发创建会话
        session_infos = self._concurrent_create_sessions(
            session_infos, scenario, config, dates_data
        )
    ...
    def close(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(self.session_map)) as executor:
            futures = [
                executor.submit(_close_single, sid, url)
                for sid, url in self.session_map.items()
            ]
